...
def validate_and_split_customer_data(
    ...
    flag(~df["Membership Type"].isin(VALID_MEMBERSHIPS), "invalid membership type")
    flag(~df["Gender"].isin(VALID_GENDERS), "invalid gender")
    flag(
        ~df["Satisfaction Level"].isin(VALID_SATISFACTION_LEVELS),
